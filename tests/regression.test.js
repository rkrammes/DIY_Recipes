...
import { toMatchImageSnapshot } from 'jest-image-snapshot'; // Import the matcher

// Extend Jest with the snapshot matcher
...
describe('Regression Tests', () => {
  ...
  const setTheme = async (themeName) => {
    ...
    await page.waitForTimeout(100); // Give a moment for styles to apply
  };
  ...
  describe('Visual Regression Tests - Theming', () => {
    const themes = ['synthwave-noir', 'terminal-mono', 'paper-ledger'];

    themes.forEach(theme => {
      test(`Home page renders correctly in ${theme} theme`, async () => {
        ...
        expect(image).toMatchImageSnapshot({
          ...
          failureThreshold: 0.01, // Allow a small threshold for minor differences
          failureThresholdType: 'percent',
        });
      });

      // Add more tests for other key components/pages in different themes
      // For example, a recipe details page, settings panel, etc.
      // test(`Recipe Details page renders correctly in ${theme} theme`, async () => {
      //   // Navigate to a recipe details page
      //   // await page.goto('http://localhost:3000/recipes/some-recipe-id', { waitUntil: 'networkidle0' });
      //   // await setTheme(theme);
      //   // const image = await page.screenshot();
      //   // expect(image).toMatchImageSnapshot({
      //   //   customSnapshotIdentifier: `recipe-details-${theme}`,
