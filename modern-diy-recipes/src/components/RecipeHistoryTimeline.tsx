...
export default function RecipeHistoryTimeline({
  ...
  return (
    <div className="container mx-auto px-4 py-6 bg-[var(--surface-0)] text-[var(--text-primary)] shadow-md rounded-lg border border-[var(--border-subtle)]">
      <h3 className="text-xl font-bold mb-6 text-[var(--text-primary)]">Recipe History Timeline</h3>
      <ul className="relative border-l border-[var(--border-subtle)] space-y-8">
        {iterations
          .sort((a, b) => new Date(a.created_at).getTime() - new Date(b.created_at).getTime())
          .map((iteration) => (
            <li key={iteration.id} className="ml-6">
              <span className="absolute flex items-center justify-center w-6 h-6 bg-[var(--accent)] rounded-full -left-3 ring-8 ring-[var(--surface-0)]">
                <svg className="w-2.5 h-2.5 text-[var(--text-inverse)]" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 20">
              ...
              {iteration.notes && (
                <p className="text-base font-normal text-[var(--text-secondary)]">{iteration.notes}</p>
              )}
            </li>
          ))}
        </ul>
