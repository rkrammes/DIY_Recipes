 ...
 * Icons are rendered based on the current theme (hackers, dystopia, neotopia)
 * to maintain consistent visual identity across the terminal UI.
 */
...
/**
 * Library icon component that adapts to the current theme
 */
export const LibraryIcon: React.FC<IconProps> = ({ size = 24, className = '' }) => {
  const { theme } = useTheme();
  // Local state to force re-render
  const [strokeColor, setStrokeColor] = useState(themeColors[theme] || themeColors.hackers);
  
  // Update color when theme changes
  useEffect(() => {
    setStrokeColor(themeColors[theme] || themeColors.hackers);
  }, [theme]);
...
/**
 * Settings icon component that adapts to the current theme
 */
...
/**
 * Recipe icon component that adapts to the current theme
 */
...
/**
 * Ingredient icon component that adapts to the current theme
 */
...
/**
 * Tool icon component that adapts to the current theme
 */
...
/**
 * FileIcon component that adapts to the current theme
 */
...
export const ThemeIcon: React.FC<{ type: string; size?: number; className?: string }> = ({
  ...
  const [strokeColor, setStrokeColor] = useState(themeColors[theme] || '#FFFFFF');
  
  // Update color when theme changes
  useEffect(() => {
    setStrokeColor(themeColors[theme] || '#FFFFFF');
  }, [theme]);
  ...
  return (
    ...
    >
      ...
      <text x="12" y="16" textAnchor="middle" fontSize="14" fill={strokeColor}>?</text>
    </svg>
  );
};
