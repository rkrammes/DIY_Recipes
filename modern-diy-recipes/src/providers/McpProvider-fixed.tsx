...
import { getClientMcpAdapter, McpAdapterType, McpAdapter } from '../lib/mcp/client';
...
export function McpProvider({ 
  ...
  const initGithubAdapter = async () => {
    try {
      const adapter = getClientMcpAdapter('github');
  ...
  const initPuppeteerAdapter = async () => {
    try {
      const adapter = getClientMcpAdapter('puppeteer');
      await adapter.connect();
  ...
  const initSupabaseAdapter = async () => {
    try {
      const adapter = getClientMcpAdapter('supabase');
      await adapter.connect();
  ...
  const initVercelAdapter = async () => {
    try {
      const adapter = getClientMcpAdapter('vercel');
      await adapter.connect();
