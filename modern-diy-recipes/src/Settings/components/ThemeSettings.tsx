...
export default function ThemeSettings() {
  ...
  const themeOptions = useMemo(() => {
    return Object.entries(themeInfo).map(([themeId, info]) => {
      const isHackers = themeId === 'hackers';
      const isDystopia = themeId === 'dystopia';
      const isNeotopia = themeId === 'neotopia';

      const backgroundColor = isHackers ? '#0a0e12' : isDystopia ? '#0a1a0a' : '#f5f5f5';

      // Memoize the theme-specific background patterns
      const themePattern = (() => {
        if (isHackers) {
          return {
  ...
  // Memoize theme characteristics to prevent re-renders
  const themeCharacteristics = useMemo(() => {
    if (theme === 'hackers') {
      return (
        <>
